    ...
    public class Carrinho
    {
        ...
        public void Ler(){
            foreach(Produto item in carrinho){
                Console.WriteLine($"R$ {item.Nome} - R$ {item.Preco}");
            }
        }

        public void Alterar(int _cod, Produto _produtoAlterado){
            carrinho.Find(f => f.Codigo == _cod).Nome  = _produtoAlterado.Nome;
            carrinho.Find(f => f.Codigo == _cod).Preco = _produtoAlterado.Preco;
